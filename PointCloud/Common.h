...
//=======
	...
	// 线相交
	static bool LineInterset(CLine &line1, CLine &line2);
	...
	/*************************更新活动链表*************************************/	
	// 最佳点是自由点
	static void UpdateMode(std::vector<CLine> &ActiveE, CLine CurrentE, Point bestP, std::vector<CLine> &InnerE, std::vector<Point> &FreeP, std::vector<Point> &ActiveP);
	
	// 最佳点位于活动边上且为当前活动边前相邻边的端点
	static void UpdateMode1(std::vector<CLine> &ActiveE, CLine CurrentE, Point bestP, std::vector<CLine> &InnerE, std::vector<Point> &ActiveP, std::map<Point, bool> &flag);
